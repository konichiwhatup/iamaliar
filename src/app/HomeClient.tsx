...
import type { HomeHero } from "@/types/product";
...
export default function HomeClient({ hero }: { hero?: HomeHero }) {
  useReveal();
  ...
  const heroImageOpacity =
    typeof hero?.heroImageOpacity === "number" ? hero.heroImageOpacity : 0.5;

  const captionInner = hero?.heroCaption ? (
    <span
      style={{
  ...
  return (
    <>
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        {/* Optional Sanity-managed hero image, layered above gradient */}
        {hero?.heroImage && (
          <div className="absolute inset-0">
            <Image
              src={hero.heroImage}
              alt={hero.heroCaption ?? "Featured"}
              fill
              priority
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(180deg, rgba(10,9,8,0.35) 0%, rgba(10,9,8,0.15) 40%, rgba(10,9,8,0.55) 100%)",
              }}
            />
          </div>
        )}
        ...
        <div ref={heroRef} className="relative z-10 text-center">
          ...
          {/* Sanity-managed caption (e.g. "Spring 2026 Selection") */}
          {captionInner && (
            <div
              style={{
                marginTop: "1.75rem",
                animation: "fadeUp 1.8s cubic-bezier(0.22,1,0.36,1) 2.8s both",
              }}
            >
              {hero?.heroLinkSlug ? (
                <Link
                  href={`/collection/${hero.heroLinkSlug}`}
